...
:: Desktop Pet (Electron ~300 MB - mostrar progresso pra nao parecer travado)
if not exist "%IDIR%\pet\package.json" goto SkipPet
echo      Instalando Desktop Pet (baixando Electron ~300 MB, 1-3 min)...
cd /d "%IDIR%\pet"
call :RPATH
call npm install --no-audit --no-fund --loglevel=error
if errorlevel 1 (
    echo      [AVISO] Desktop Pet falhou - JARVIS funciona sem ele
    echo      Para tentar depois: cd pet ^&^& npm install
    echo [%time%] S7 PET FAIL >> "%LOGF%"
) else (
    echo      [OK] Desktop Pet
)
:SkipPet
